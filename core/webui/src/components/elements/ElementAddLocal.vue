...
let projectsLoaded = $ref(false);
let branches = ref<string[] | string>([]);
let branchesLoaded = $ref(false);
  ...
  <n-space vertical>
    ...
    <n-steps vertical :current="(currentRef as number)" status="process">
      <n-step title="Wybierz projekt">
        ...
        <template v-if="currentRef === 1 && projects.length">
          <n-button
            ...
            :key="id"
            class="addBrick"
            _click="chooseProject(project)"
          >
            {{ project }}
          </n-button>
        </template>
        <p
          v-if="projectsLoaded && currentRef === 1 && !projects.length"
          style="color: red"
        >
          Brak projektu
        </p>
      ...
      <n-step title="Wybierz gałąź">
        <template v-if="currentRef === 2 && branches.length">
          <n-button
        ...
        <p
          v-if="branchesLoaded && currentRef === 2 && !branches.length"
          style="color: red"
        >
          Brak gałęzi
        </p>
